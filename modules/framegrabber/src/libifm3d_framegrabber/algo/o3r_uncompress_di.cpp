                         ...
                         uint16_t width, uint16_t height)
{
    ...
    /* Note: if necessary, the rotated unit vectors might be cached if 
             intrModelID, intrModelParameters, extrRot[XYZ], width and height do not change.
             This is not performed here for simplicity. */

    if( 0 != evalIntrinsic(x, y, z, intrModelID, intrModelParameters, width, height) )
    {
        return 1;
    }
    ...
    for(idx = 0; idx < (uint32_t)width*(uint32_t)height; idx++)
    {
        ...
        vy = y[idx];
        vz = z[idx];
        x[idx] = (u16Dist[idx] == 0) ? 0.0f : (d * (R[0][0] * vx + R[0][1] * vy + R[0][2] * vz) + extrTransX);
        y[idx] = (u16Dist[idx] == 0) ? 0.0f : (d * (R[1][0] * vx + R[1][1] * vy + R[1][2] * vz) + extrTransY);
        z[idx] = (u16Dist[idx] == 0) ? 0.0f : (d * (R[2][0] * vx + R[2][1] * vy + R[2][2] * vz) + extrTransZ);
        dist[idx] = d;
    }
    return 0;
}
