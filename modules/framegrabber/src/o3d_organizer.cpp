                              ...
                              const bool masking)
{
  ...
  if (chunks.find(ImageChunk::CARTESIAN_ALL) != chunks.end())
    {
      size_t const cart_all_idx = *(chunks[ImageChunk::CARTESIAN_ALL].begin());
      ...
      chunks[ImageChunk::CARTESIAN_X_COMPONENT] = {x_idx};
      chunks[ImageChunk::CARTESIAN_Y_COMPONENT] = {y_idx};
      chunks[ImageChunk::CARTESIAN_Z_COMPONENT] = {z_idx};
      if (requested_images.count(
            static_cast<buffer_id>(buffer_id::CARTESIAN_ALL)) == 0)
        {
          chunks.erase(ImageChunk::CARTESIAN_ALL);
        }
    }
