...
namespace ifm3d
{
  ...
  {
    return std::get<std::vector<XMLRPCValue>>(*_value);
  }

  std::vector<std::uint8_t> const&
  XMLRPCValue::AsByteArray() const
  {
    return std::get<std::vector<std::uint8_t>>(*_value);
  }

  std::unordered_map<std::string, XMLRPCValue> const&
  XMLRPCValue::AsMap() const
  {
    return std::get<std::unordered_map<std::string, XMLRPCValue>>(*_value);
  }
  ...
  {
    return this->_xmlrpc_port;
  }

  XMLRPCValue
  XMLRPC::parse_xmlrpc_response(const std::string& xml_response)
  {
  ...
  std::string
  XMLRPC::create_xmlrpc_request(const std::string& method_name,
