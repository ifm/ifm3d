...
{
  EXPECT_NO_THROW(this->_dev->FactoryReset());
...
{
  EXPECT_EQ(this->_dev->Password(), ifm3d::DEFAULT_PASSWORD);
}

TEST_F(LegacyDeviceTest, ApplicationList)
{
  ifm3d::json app_list = this->_dev->ApplicationList();
...
TEST_F(LegacyDeviceTest, SessionManagement)
{
  ...
  std::string sid = this->_dev->RequestSession();
  EXPECT_STREQ(sid.c_str(), this->_dev->SessionID().c_str());
  bool const retval = this->_dev->CancelSession();
  EXPECT_TRUE(retval);
  EXPECT_STREQ("", this->_dev->SessionID().c_str());

  // explicitly create but implicitly cancel (via dtor) session
  sid = this->_dev->RequestSession();
  EXPECT_STREQ(sid.c_str(), this->_dev->SessionID().c_str());
  this->_dev = std::make_shared<ifm3d::LegacyDevice>();
  EXPECT_STREQ("", this->_dev->SessionID().c_str());
  ...
  if (!ifm3d::DEFAULT_SESSION_ID.empty())
    {
      ...
      EXPECT_TRUE(this->_dev->CancelSession());
      ...
      // New scope, so the `cam2` dtor runs
      {
        ...
        EXPECT_THROW(this->_dev->RequestSession(), ifm3d::Error);
        // Let's now use `this->cam_` to cancel the sid from the other
        // camera
        EXPECT_TRUE(this->_dev->CancelSession(sid));
...
{
  if (this->_dev->AmI(ifm3d::LegacyDevice::device_family::O3X))
    {
      return;
    }
  ...
  int const idx = this->_dev->CopyApplication(1);
  app_list = this->_dev->ApplicationList();
  EXPECT_EQ(app_list.size(), 2);

  this->_dev->DeleteApplication(idx);
  app_list = this->_dev->ApplicationList();
  EXPECT_EQ(app_list.size(), 1);
}
...
{
  EXPECT_THROW(this->_dev->CopyApplication(-1), ifm3d::Error);
  EXPECT_THROW(this->_dev->DeleteApplication(-1), ifm3d::Error);
}
...
TEST_F(LegacyDeviceTest, CreateDeleteApplication)
{
  ...
  int idx = -1;
  std::vector<std::string> const app_types = this->_dev->ApplicationTypes();
  for (const auto& s : app_types)
    {
      idx = this->_dev->CreateApplication(s);
      app_list = this->_dev->ApplicationList();
      EXPECT_EQ(app_list.size(), 2);
...
{
  EXPECT_THROW(this->_dev->CreateApplication("Foo"), ifm3d::Error);
}
...
TEST_F(LegacyDeviceTest, ImportExportApplication)
{
  ...
  if (this->_dev->AmI(ifm3d::LegacyDevice::device_family::O3X))
    {
      EXPECT_NO_THROW(bytes = this->_dev->ExportIFMApp(idx));
      EXPECT_NO_THROW(new_idx = this->_dev->ImportIFMApp(bytes));
  ...
  else
    {
      ...
      this->_dev->DeleteApplication(new_idx);
      app_list = this->_dev->ApplicationList();
      EXPECT_EQ(app_list.size(), 1);
    }
}
...
TEST_F(LegacyDeviceTest, ImportExportConfig)
{
  ...
  EXPECT_NO_THROW(bytes = this->_dev->ExportIFMConfig());
  EXPECT_NO_THROW(this->_dev->ImportIFMConfig(
    bytes,
...
TEST_F(LegacyDeviceTest, ActiveApplication)
{
  ...
  //
  if (this->_dev->AmI(ifm3d::LegacyDevice::device_family::O3X) ||
      this->_dev->AmI(ifm3d::LegacyDevice::device_family::O3R))
    {
      return;
    }

  // create a new application using JSON syntax
  EXPECT_NO_THROW(this->_dev->FromJSONStr(R"({"Apps":[{}]})"));
  ...
  EXPECT_NO_THROW(
    this->_dev->FromJSONStr(R"({"Device":{"ActiveApplication":"2"}})"));
  app_list = this->_dev->ApplicationList();
  for (auto& a : app_list)
    {
      if (a["Active"].get<int>())
  ...
  EXPECT_NO_THROW(this->_dev->DeleteApplication(idx));
  ...
  EXPECT_NO_THROW(
    this->_dev->FromJSONStr(R"({"Device":{"ActiveApplication":"1"}})"));
  app_list = this->_dev->ApplicationList();
  for (auto& a : app_list)
    {
      if (a["Active"].get<int>())
...
{
  if (this->_dev->AmI(ifm3d::LegacyDevice::device_family::O3R))
    {
      return;
    }
  ifm3d::json dump = this->_dev->ToJSON();
  std::string const curr_im_type = dump["ifm3d"]["Apps"][0]["Imager"]["Type"];

  std::vector<std::string> const im_type = this->_dev->ImagerTypes();
  ifm3d::json j = R"({"Apps":[]})"_json;
  for (const auto& it : im_type)
    {
      dump["ifm3d"]["Apps"][0]["Imager"]["Type"] = it;
      j["Apps"] = dump["ifm3d"]["Apps"];
      this->_dev->FromJSON(j);
      dump = this->_dev->ToJSON();
      EXPECT_STREQ(
  ...
  j["Apps"] = dump["ifm3d"]["Apps"];
  EXPECT_NO_THROW(this->_dev->FromJSON(j));
}

TEST_F(LegacyDeviceTest, Filters)
{
  // factory defaults, should only be 1 application
  ifm3d::json const app_list = this->_dev->ApplicationList();
  EXPECT_EQ(app_list.size(), 1);
  ...
  EXPECT_NO_THROW(this->_dev->FromJSONStr(j));
...
{
  EXPECT_NO_THROW(this->_dev->FromJSON(this->_dev->ToJSON()));
  EXPECT_NO_THROW(this->_dev->FromJSONStr(this->_dev->ToJSONStr()));
...
TEST_F(LegacyDeviceTest, Time)
{
  ...
  EXPECT_NO_THROW(this->_dev->FromJSON(dump));
  dump = this->_dev->ToJSON();
  n_tries =
    std::stoi(dump["ifm3d"]["Time"]["WaitSyncTries"].get<std::string>());
  ...
  //
  EXPECT_NO_THROW(this->_dev->SetCurrentTime());
}
...
TEST_F(LegacyDeviceTest, TemporaryParameters)
{
  ...
  EXPECT_NO_THROW(_dev->SetTemporaryApplicationParameters(params));
