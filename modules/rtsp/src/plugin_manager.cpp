...
proc_handle_t get_symbol(void* handle, const char* name)
{
#ifdef _WIN32
...
bool matches_plugin_name(const std::filesystem::path& p)
{
...
PluginManager::PluginManager(const std::vector<std::string>& paths,
                             const std::string& preferred_plugin_name)
  : config_paths_(paths), preferred_plugin_name_(preferred_plugin_name)
{}

PluginManager::~PluginManager()
{
  {
    std::lock_guard<std::mutex> lock(decoder_map_mutex_);
    if (!decoder_owners_.empty())
    {
      LOG_WARNING("[PluginManager] WARNING: {} decoder(s) still alive at PluginManager shutdown.", decoder_owners_.size());
    }
  }
...
std::vector<std::string> PluginManager::CollectSearchPaths() const
{
...
void PluginManager::ScanDirectory(const std::string& path)
{
  ...
  if (ec)
  {
    LOG_ERROR("[PluginManager] Cannot scan directory: {} ({})", path, ec.message());
    return;
  }
...
{
  if(std::any_of(loaded_plugins_.begin(), loaded_plugins_.end(),
  ...
  if (!handle)
  {
    LOG_ERROR("[PluginManager] Failed to open: {} ({})", path, last_dl_error());
    return false;
  }
  ...
  if (!sym)
  {
    LOG_ERROR("[PluginManager] Missing symbol 'video_plugin_init' in: {} ({})", path, last_dl_error());
    close_library(handle);
    return false;
  }

  video_plugin_init_fn init_fn = reinterpret_cast<video_plugin_init_fn>(sym);
  ...
  {
    LOG_ERROR("[PluginManager] ABI version mismatch in: {} (plugin={}, host={})",
              path, api->abi_version, VIDEO_PLUGIN_ABI_VERSION);
    close_library(handle);
    return false;
  }

  if (!api->create_decoder || !api->destroy_decoder ||
      !api->send_packet || !api->receive_frame)
  {
    LOG_ERROR("[PluginManager] Plugin missing required function pointers: {}", path);
    close_library(handle);
    return false;
  }
  ...
  if (!probe)
  {
    LOG_WARNING("[PluginManager] Plugin does not support H264 (probe failed): {}", path);
...
const VideoPluginAPI* PluginManager::GetPluginApi(VideoCodec codec) const
{
  auto it = codec_support_.find(codec);
  return (it != codec_support_.end()) ? it->second : nullptr;
}

VideoDecoder* PluginManager::CreateDecoder(VideoCodec codec)
{
  auto it = codec_support_.find(codec);
...
void PluginManager::DestroyDecoder(VideoDecoder* decoder)
{
  if (!decoder)
  {
    return;
  ...
  const VideoPluginAPI* api = nullptr;
  {
    ...
    {
      LOG_ERROR("[PluginManager] DestroyDecoder: unrecognised decoder pointer");
      return;
    }
    api = it->second;
