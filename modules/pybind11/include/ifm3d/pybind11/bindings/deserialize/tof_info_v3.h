...
bind_struct_tofinfov3(pybind11::module_& m)
{
  py::class_<ifm3d::calibration::ExtrinsicOpticToUser,
             ifm3d::calibration::ExtrinsicOpticToUser::Ptr>
    extrinsic_optic_to_user(m,
                            "ExtrinsicOpticToUser",
                            R"(
          Class for managing an instance Extrinsic parameters
       )");
  extrinsic_optic_to_user.def(py::init<>(),
                              R"(
        Constructor
      )");
  ...
  py::class_<ifm3d::calibration::Calibration,
             ifm3d::calibration::Calibration::Ptr>
    calibration(m,
                "Calibration",
                R"(
        Class for managing an instance calibration parameters
      )");
  calibration.def(py::init<>(),
                  R"(
        Constructor
      )");
  ...
  py::class_<ifm3d::TOFInfoV3, ifm3d::TOFInfoV3::Ptr> tof_info_v3(m,
                                                                  "TOFInfoV3",
                                                                  R"(
        Class for managing an instance of an struct TofInfoV3
      )");

  tof_info_v3.def(py::init<>(),
                  R"(
        Constructor
      )");
  ...
  tof_info_v3.def_readonly("extrinsic_optic_to_user",
