...
{
  py::class_<ifm3d::ODSOccupancyGridV1, ifm3d::ODSOccupancyGridV1::Ptr>
    ods_occupancy_grid_v1(m,
                          "ODSOccupancyGridV1",
                          R"(
        Class for managing an instance of an struct ODSOccupancyGridV1
      )");

  ods_occupancy_grid_v1.def(py::init<>(),
                            R"(
        Constructor
      )");

  ods_occupancy_grid_v1.def_readonly("timestamp_ns",
                                     &ifm3d::ODSOccupancyGridV1::timestamp_ns,
                                     R"(
        timestamp of the grid 
      )");

  ods_occupancy_grid_v1.def_readonly("width",
                                     &ifm3d::ODSOccupancyGridV1::width,
                                     R"(
        number of grid cells
      )");

  ods_occupancy_grid_v1.def_readonly("height",
