...
static size_t SWupdateStatusWriteCallbackIgnore(char* ptr, size_t size, size_t nmemb, void* userdata)
  {
  return size * nmemb;
  }

static size_t SWupdateStatusWriteCallback(char* ptr, size_t size, size_t nmemb, void* userdata)
  {
...
void ifm3d::SwupdateApp::checkRecovery()
  ...
  if(curlResult != CURLE_OK || statusCode != 200)
    ...
    {
    std::cout << "Waiting for the SWUpdate process to become ready" << std::endl;
...
void ifm3d::SwupdateApp::uploadData(std::shared_ptr<std::istream> data, size_t filesize)
  ...
  curl_easy_setopt(curl, CURLOPT_URL, createURL(this->cam_->IP(), FWU_RECOVERY_PORT, FWU_UPLOAD_URL).c_str());
  ...
  while(!updateFinished)
    ...
    if(statusId == FWU_STATUS_FAILURE)
      ...
      if(statusMessage != "ERROR parser/parse_config.c : parse_cfg")
        {
        std::cout << "Update failed with status: \"" << statusMessage << "\"" << std::endl;
...
int ifm3d::SwupdateApp::checkRecoveryMode()
  ...
  try
    ...
    {
    /* checking for production mode if not able to connect to camera it will throw connection error*/
    try
      {
...
int ifm3d::SwupdateApp::reboottoProductiveMode()
  ...
  try
    ...
    checkRecovery();
    reboot();
       std::cout << "Device rebooted to Productive mode from Recovery mode " << std::endl;
    return 0;
    }
  catch(const ifm3d::error_t  &ex)
    {
